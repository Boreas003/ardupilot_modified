...
float AC_PID::update_all_depth(float target, float measurement, bool limit)
{
	...
	if (_flags._reset_filter) {
		_flags._reset_filter = false;
		_target = target;
		_error = -1.0f*(_target - measurement);
		_derivative = 0.0f;
	} else {
		float error_last = _error;
		_target += get_filt_T_alpha() * (target - _target);
		_error += get_filt_E_alpha() * (-1.0f*(_target - measurement) - _error);
...
float AC_PID::update_all_yaw(float target, float measurement, bool limit)
{
    // don't process inf or NaN
    if (!isfinite(target) || !isfinite(measurement)) {
        return 0.0f;
    ...
    return (P_out + _integrator + D_out)*0.33f;
}

// yaw for backward
float AC_PID::update_all_yaw_back(float target, float measurement, bool limit)
{
    // don't process inf or NaN
    if (!isfinite(target) || !isfinite(measurement)) {
        return 0.0f;
